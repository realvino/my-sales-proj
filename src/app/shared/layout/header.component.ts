...
export class HeaderComponent extends AppComponentBase implements OnInit {
    ...
    @ViewChild('closedDealModal') closedDealModal:ClosedDealNotificationComponent;
    ...
    shownTenantName: string = "";
    profilePicture: string = "/assets/common/images/default-profile-picture.png";
    recentlyLinkedUsers: LinkedUserDto[];
    unreadChatMessageCount = 0;
	items :Array<any>;
	 Math: any;
   public monthAchived : number;
   public monthTarget : number;
   public yearAchived : number;
   public yearTarget : number; 
    ...
    constructor(
		...
		private _userTargetService:TenantDashboardServiceProxy
    ) {
        super(injector);
    }
    ...
    getCurrentLoginInformations(): void {
        ...
        this.shownTenantName = this.appSession.getShownTenantName();
	...
	getUserTarget():void{
		...
		this._userTargetService.getUserTarget(0).subscribe((result) => {
			items=result;
			this.SpResultUserTarget = items[0];
			
			/*this.monthAchived = this.SpResultUserTarget.monthAchived;
			this.monthTarget = this.SpResultUserTarget.monthTarget;
            this.yearAchived = this.SpResultUserTarget.yearAchived;
			this.yearTarget = this.SpResultUserTarget.yeartarget;
			this.monthtarget = (this.monthAchived)/(this.monthTarget );
			this.yeartarget = (this.yearAchived)/(this.yearTarget);
			this.monthpercentage = (this.monthtarget)/100;
            this.yearpercentage = (this.yeartarget)/100;*/
            // this.monthAchived = 0;
            // this.monthTarget = 0;
            // this.yearAchived = 0;
            // this.yearTarget =  0;
            ...
            this.monthAchived = items[0].monthAchived;
            this.monthTarget = items[0].monthTarget;
            this.yearAchived = items[0].yearAchived;
            this.yearTarget =  items[0].yeartarget;
            this.monthtarget = (this.monthAchived)/(this.monthTarget);
			this.yeartarget = (this.yearAchived)/(this.yearTarget);
			this.monthpercentage = Math.round((this.monthtarget)*100);
            this.yearpercentage = Math.round((this.yeartarget)*100);
        });
